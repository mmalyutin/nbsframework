...
public class FieldGenerator extends AbstractGenerator {
	...
	return getGenerator(data).generateClass(context, data);
    }

    @Override
    public String getClassName(String baseType, String type) {
	return getGenerator(baseType, type).getClassName(baseType, type);
    }
    
    @Override
    public void generatePopulate(ScopeContext context, IData data, String bean, SourceWriter sw) {
	getGenerator(data).generatePopulate(context, data, bean, sw);
    }
