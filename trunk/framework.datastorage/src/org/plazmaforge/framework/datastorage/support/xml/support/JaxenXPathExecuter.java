...
public class JaxenXPathExecuter implements XPathExecuter {

    private final Map<String, XPath> cachedXPaths = new HashMap<String, XPath>(); //new ReferenceMap(); //soft cache
	
    public JaxenXPathExecuter() {
    }

    protected XPath getXPath(String expression) throws DSException {
	XPath xPath = cachedXPaths.get(expression);
	if (xPath == null) {
	    try {
		xPath = new DOMXPath(expression);
	    } catch (JaxenException e) {
		throw new DSException("XPath compilation failed. Expression: "	+ expression, e);
	    }
