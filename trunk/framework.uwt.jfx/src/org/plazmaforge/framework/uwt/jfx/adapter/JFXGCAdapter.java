 ...
 *
 */
public class JFXGCAdapter extends JFXAbstractAdapter {
	...
	GC gc = (GC) element;
	javafx.scene.canvas.GraphicsContext xGC = (javafx.scene.canvas.GraphicsContext) element.getDelegate();
	if (xGC == null) {
	    return null;
	}
	...
	if (eq(methodName, GC.METHOD_SET_BACKGROUND)) {
	    ...
	    if (args.length == 1) {
		 ...
		 return getStringWidth(xGC, string);
	    }
	    return null;
	    ...
	    if (args.length == 1) {
		String string = (String) args[0];
		int width = getStringWidth(xGC, string);
	...
	//Draw    
		...
		javafx.scene.paint.Paint fillStyle = xGC.getFill();
		javafx.scene.paint.Paint strokeStyle = xGC.getStroke();
		...
		xGC.setFill(strokeStyle);
		xGC.fillText(text, x, y + fontHeight);
		xGC.closePath();
		xGC.fill();
		
		// Restore fill style
		xGC.setFill(fillStyle);
    ...
    protected void drawEllipse(javafx.scene.canvas.GraphicsContext xGC, int x, int y, int width, int height) {
    ...
    protected void drawArc(javafx.scene.canvas.GraphicsContext xGC, int x, int y, int width, int height, int startAngle, int endAngle, boolean fill) {
	double rx = width / 2;
	double ry = height / 2;
	double ox = x + rx;
	double oy = y + ry;
    ...
    */
    ...
    protected int getFontSize(javafx.scene.text.Font font) {
	//TODO
	if (font == null) {
	    return 0;
	}

	//float width = com.sun.javafx.tk.Toolkit.getToolkit().getFontLoader().computeStringWidth(string, font);
	//float height = com.sun.javafx.tk.Toolkit.getToolkit().getFontLoader().getFontMetrics(font).getLineHeight();
	
	//Bounds bounds = TextBuilder.create().text(text).font(font).build().getLayoutBounds();
	//double width=bounds.getWidth();
	//double height=bounds.getHeight();
	
	return (int) com.sun.javafx.tk.Toolkit.getToolkit().getFontLoader().getFontMetrics(font).getLineHeight();
    }

    protected int getStringWidth(javafx.scene.text.Font font, String string) {
	return (int) com.sun.javafx.tk.Toolkit.getToolkit().getFontLoader().computeStringWidth(string, font);
    }

    protected int getStringWidth(javafx.scene.canvas.GraphicsContext xGC, String string) {
	return getStringWidth(xGC.getFont(), string);
    }
