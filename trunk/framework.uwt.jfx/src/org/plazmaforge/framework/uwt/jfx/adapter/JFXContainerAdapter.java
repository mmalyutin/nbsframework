...
public class JFXContainerAdapter extends JFXControlAdapter {
	...
	container.resetInitProperties(Container.PROPERTY_LAYOUT);
	return xContainer;
    }
    ...
    protected boolean isLayoutContainer(Object delegate) {
	return delegate instanceof XLayoutContainer;
    }

    protected XLayoutContainer asLayoutContainer(Object delegate) {
	return (XLayoutContainer) delegate;
    }
...
//    }
	...
	javafx.scene.Parent content = getContent(delegate);
	if (content == null) {
	    return;
	}
	XLayoutContainer xContainer = null;
	if  (isLayoutContainer(content)) {
	    xContainer = asLayoutContainer(content);
	} else {
	    
	    // Get wrapper: parent
	    if (isLayoutContainer(content.getParent())) {
		 xContainer = asLayoutContainer(content.getParent());
	    }
	}
	...
	if (Container.PROPERTY_LAYOUT.equals(name)) {
	    ...
	    //TODO: Implement reset layout and set new panel
