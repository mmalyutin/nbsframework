 ...
 *
 */
public class CheckBoxGenerator extends AbstractFieldGenerator {
    
    @Override
    public boolean accept(String type) {
	return eq(type, UIBuilder.CHECK_BOX_TYPE);
    }
	...
	generateSetRSStringProperty(context, data, bean, CheckBox.PROPERTY_TEXT, sw);
    }
    
    @Override
    protected void generateSetValue(ScopeContext context, IData data, String bean, SourceWriter sw) {
	generateSetBooleanProperty(context, data, bean, CheckBox.PROPERTY_VALUE, sw);
	generateSetBooleanProperty(context, data, bean, CheckBox.PROPERTY_SELECTED, sw); // 'selected' attribute overrides value
    }    
