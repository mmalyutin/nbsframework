...
public class XLink extends JButton {
    ...
    public XLink(Action action) {
      this();
      setAction(action);
    }

    public XLink(Icon icon) {
      this(null, icon, null);
    }

    public XLink(String s) {
      this(s, null, null);
    }
    ...
    public XLink(String s, URL url) {
      this(s, null, url);
    }

    public XLink(Icon icon, URL url) {
      this(null, icon, url);
    }

    public XLink(String text, Icon icon, URL url) {
      super(text, icon);
  ...
  class BasicLinkButtonUI extends MetalButtonUI {
    ...
    protected void paintText(Graphics g, JComponent com, Rectangle rect,
      ...
      if (behaviour == XLink.HOVER_UNDERLINE) {
        if (bnModel.isRollover())
          drawLine = true;
      } else if (behaviour == XLink.ALWAYS_UNDERLINE || behaviour == XLink.SYSTEM_DEFAULT)
        drawLine = true;
      if (!drawLine)
        return;
